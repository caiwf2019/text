...
import {
  Form, Row, Col, Button, Select,Card,message,InputNumber
} from 'antd';
...
class Index extends Component {
  ...
  getFields() {
    ...
    children.push(
      <Col key="searchsclear" span={6} style={{ textAlign: 'center', marginTop:5 }}>
        ...
        <Button style={{marginLeft:10}} onClick={this.handleReset}>Clear</Button>
      </Col>
    );
    return children;
  }
  ...
  handleCSV =()=>{
    ...
    if(getAnalyze[0]){
      getAnalyze.forEach(v=>{
        csvArr+=`\n${v.h},${v.H[0]},${v.H[1]},${zone[0].zones[v.z].width.maximum},${zone[0].zones[v.z].width.minimum},`;
      });
  ...
  handleCSVHigth =()=>{
    ...
    if(getAnalyze[0]) {
      ...
      getAnalyze.forEach(v => {
        csvArr += `\n${v.h},${v.H[0]},${v.H[1]},${zone[0].zones[v.z].height.maximum},${zone[0].zones[v.z].height.minimum},`;
      });
      const blob = new Blob([csvArr], {type: 'text/plain;charset=utf-8'});
      saveAs(blob, thisDate);
    }
  ...
  handleCSVV =()=>{
    ...
    if(getAnalyze[0]) {
      const thisDate = `高度${this.getTimeFormatter()}.csv`;
      getAnalyze.forEach(v => {
        csvArr += `\n${v.v},${v.V[0]},${v.V[1]},${zone[0].zones[v.z].volume.maximum},${zone[0].zones[v.z].volume.minimum},`;
      });
      const blob = new Blob([csvArr], {type: 'text/plain;charset=utf-8'});
      saveAs(blob, thisDate);
    }
  ...
  render() {
    ...
    return (
      <Card>
        ...
        <Card title="宽度" bordered={false} extra={<Button onClick={()=>this.handleCSV()}>导出</Button>}>
        ...
        <Card title="高度" bordered={false} extra={<Button onClick={()=>this.handleCSVHigth()}>导出</Button>}>
          <Suspense fallback={null}>
            <OfflineData
              loading={loading}
        ...
        <Card title="体积" bordered={false} extra={<Button onClick={()=>this.handleCSVV()}>导出</Button>}>
          <Suspense fallback={null}>
            <OfflineData
              loading={loading}
        ...
        <Card title="合格率" bordered={false} extra={<Button onClick={()=>this.handleHeGeCSV()}>导出</Button>}>
          <Row>
            <Col span={12}>
